...
class Bot_Caronas():
    ...
    def add(self, bot, update, args, user_data, chat_data):
        ...
        if int(time_hour) < 19 and int(horario_add.split(':')[0]) > 21 and week_day != 6:
            update.message.reply_text('Só é possível oferecer caronas a partir das 19h.')

        else:

            if sentido_add == 'ida':
                for horario, username, user_id, descriçao, vagas,picpay in self.caronas_ida:
            ...
            if sentido_add == 'volta':
                for horario, username, user_id, descriçao, vagas,picpay in self.caronas_volta:
                    if username_add == username:
                        update.message.reply_text(
    ...
    def ida(self, bot, update):
        self.caronas_ida = sorted(self.caronas_ida,key=lambda x: x[0])
        time_hour = datetime.now().hour
        if time_hour  == 19:
            self.caronas_ida = []
        ...
        for horario, username, user_id, descricao, vagas,picpay in self.caronas_ida:
            hour = int(horario.split(':')[0])
            minute = int(horario.split(':')[1])
            if time_hour >= hour and time_min > minute:
                self.caronas_ida.remove([horario,username,user_id,descricao,vagas,picpay])
            else:
                msg += 'Horário: %s - Motorista: @%s - Descrição: %s - Vagas: %s %s'%(horario,username,descricao,str(vagas),picpay)
        bot.sendMessage(chat_id=update.message.chat_id, text=msg)

    def volta(self, bot, update):
        self.caronas_volta = sorted(self.caronas_volta,key=lambda x: x[0])
        time_hour = datetime.now().hour
        if time_hour  == 19:
            self.caronas_volta = []
        ...
        time_hour = datetime.now().hour
        time_min = datetime.now().minute
        for horario, username, user_id, descricao, vagas,picpay in self.caronas_volta:
            hour = int(horario.split(':')[0])
            minute = int(horario.split(':')[1])
            if time_hour >= hour and time_min > minute:
                self.caronas_volta.remove([horario,username,user_id,descricao,vagas,picpay])
            else:
                msg += 'Horário: %s - Motorista: @%s - Descrição: %s - Vagas: %s %s'%(horario,username,descricao,str(vagas),picpay)
    ...
    def picpay(self,bot,update,user_data,chat_data):
        user_id = update.message.from_user['id']
        for i in range(len(self.caronas_ida)):
            if self.caronas_ida[i][5] == '(P)':
                self.caronas_ida[i][5] = ' '
                update.message.reply_text('Você não está aceitando Picpay')
            else:
                self.caronas_ida[i][5] = '(P)'
                update.message.reply_text('Agora você aceita picpay.')
        for i in range(len(self.caronas_volta)):
            if self.caronas_volta[i][5] == '(P)':
                self.caronas_volta[i][5] = ' '
                update.message.reply_text('Você não está aceitando Picpay')
            else:
                self.caronas_volta[i][5] = '(P)'
                update.message.reply_text('Agora você aceita picpay.')
...
def main():
    ...
    dispatcher.add_handler(
        CommandHandler('picpay', bot_car.picpay,pass_user_data=True,
                        pass_chat_data=True)
    )
